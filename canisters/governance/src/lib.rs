...
async fn cast_vote(
    ...
    // let balance = get_balance(
    //     TOKEN_PRINCIPAL.with(|token_principal| token_principal.borrow().clone()),
    //     ic_cdk::caller()
    // ).await;
    ...
    // TODO get the current proposals
    // TODO make sure the voting principal has not already voted
    // TODO create the vote and link it to the proposal
    // TODO update the canister if the threshold is passed

    let proposal = get_proposal(proposal_id).await;

    // let allowed_to_vote = proposals.iter().any(|proposal| {
    //     return proposal.principal == ic_cdk::caller();
    // });
}
...
async fn get_proposal(id: String) -> Proposal {
